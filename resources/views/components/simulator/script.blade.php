<script>
    ...
    function calculateResult() {
        ...
        simulatedProposta.debit = proposal.debit;
        simulatedProposta.credit = proposal.credit;
        simulatedProposta.parc_2_6 = proposal.parc_2_6;
        simulatedProposta.parc_7_12 = proposal.parc_7_12;
        simulatedProposta.elo_debit = proposalDebitElo.value;
        simulatedProposta.elo_credit = proposalCreditElo.value;
        simulatedProposta.elo_parc_2_6 = proposalC26Elo.value;
        simulatedProposta.elo_parc_7_12 = proposalC712Elo.value;
        simulatedProposta.anticipation = proposal.anticipation;
        simulatedProposta.monthly_rental = proposal.monthly_rental;
    ...
    function sendToRD(e) {
        e.preventDefault();
        simulatedProposta.vendedor = document.getElementById('vendedor').value;
        simulatedProposta.deal = getDealFromUrlParams();
        simulatedProposta.merchantData = merchatData;
        console.log('DADOS PREPARADOS:', simulatedProposta)
        fetch('/api/simulator/submit', {
            method: 'POST',
            body: JSON.stringify(simulatedProposta),
            headers: {
                'Content-Type': 'application/json',
                'Api-Token': token
            }
            ...
            .then(response => response.json())
            .then(data => {
                let sendToRdMessage = document.getElementById('send_to_rd_message');
                if (data.error) {
                    sendToRdMessage.innerText = data.error;
                    if (sendToRdMessage.classList.contains('text-green-500')){
                        sendToRdMessage.classList.remove('text-green-500');
                    }
                    if (!sendToRdMessage.classList.contains('text-red-500')){
                        sendToRdMessage.classList.add('text-red-500');
                    }
                    if (sendToRdMessage.classList.contains('hidden')){
                        sendToRdMessage.classList.remove('hidden');
                    }
                } else {
                    sendToRdMessage.innerText = 'Proposta enviada com sucesso!';
                    let sendToRdButton = document.getElementById('send_to_rd');
                    let voltarNegociacoes = document.getElementById('voltar_negociacoes');
                    if (sendToRdButton.classList.contains('text-red-500')){
                        sendToRdButton.classList.remove('text-red-500');
                    }
                    if (!sendToRdButton.classList.contains('text-green-500')){
                        sendToRdButton.classList.add('text-green-500');
                    }
                    sendToRdButton.classList.add('hidden');
                    sendToRdMessage.classList.remove('hidden');
                    voltarNegociacoes.classList.remove('hidden');
                }
            });
    }

    function getDealFromUrlParams(){
        let url = window.location.href;
        let dealId = url.split('/negociacoes/')[1].split('/proposta')[0];
        return dealId;
    }
</script>
