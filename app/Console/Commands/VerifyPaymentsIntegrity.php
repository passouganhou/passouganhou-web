...
class VerifyPaymentsIntegrity extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'payments:verify {startDate?} {endDate?}';
     ...
     * @var string
     */
    protected $description = 'Verify payments integrity from GsurfService';
    ...
    public function handle(): int
    {
        ...
        try {
            ...
            $paymentsFromApi = $this->gsurfService->getFormattedPayments($startDate, $endDate);
            $paymentsFromDB = Payment::where('payment_date', '>=', $startDate)
                ->where('payment_date', '<=', $endDate)
                ->get();
            $dbPaymentsCount = count($paymentsFromDB);
            $apiPaymentsCount = count($paymentsFromApi);

            $diff = $apiPaymentsCount - $dbPaymentsCount;
            ...
            $paymentsFromDatabase = collect($paymentsFromDB);

            $this->info('Payments from API: ' . $apiPaymentsCount);
            $this->info('Payments from DB: ' . $dbPaymentsCount);
            $this->info('Difference: ' . $diff);

            $this->info('Checking for missing payments...');
            $this->output->progressStart($apiPaymentsCount);
            $paymentsFromApi->each(function ($payment) use ($paymentsFromDatabase) {
                ...
                if (!$paymentFromDatabase) {
                    $this->info('Payment not found: ' . $payment->unique_id);
                    $payment->save();
                    $this->info('Payment saved successfully.');
                }
            });
            $this->output->progressFinish();
            $this->info('Missing payments checked successfully.');
